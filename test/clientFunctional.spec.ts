...
import {
    ...
    BillInterface, PayoutRecipients
} from "../src/Model";
...
let exampleInvoice = function () {
    const invoice = new Invoice(10.00, "USD");
...
describe('BitPaySDK.Client', () => {
    beforeAll(() => {
        ...
        let configFilePath = __dirname + '/BitPay.config.json';
    ...
    describe('Rates', () => {
        it('should get rate', async () => {
            let result = await client.getRate('BCH', 'USD');
    ...
    describe('Recipients', () => {
        ...
        it("should create recipient", async() => {
            ...
            const result: PayoutRecipientInterface[] = await client.submitPayoutRecipients(new PayoutRecipients(recipientsList));
    ...
    describe('Payout', () => {
        ...
        it("should submit payout", async() => {
            let recipientsList = [];
            let payoutEmail = getEmail();
            const requestedRecipient: PayoutRecipientInterface  = new PayoutRecipient(payoutEmail, "Bob", null);
            recipientsList.push(requestedRecipient);

            const recipients: PayoutRecipientInterface[] = await client.submitPayoutRecipients(new PayoutRecipients(recipientsList));
            const payoutRecipientId = recipients[0].id;

            const payout = new Payout(10.00, "USD", "USD");
    ...
    describe('Bill', () => {
        ...
        it('should create bill', async() => {
            ...
            item1.price = 10.00;
            const requestedBill: BillInterface = new Bill(
        ...
        it('should updated bill', async() => {
            ...
            itemUpdated.price = 9.00;
            itemUpdated.quantity = 1;

            const updatedBillRequest = new Bill(
                "bill1234-ABCD", "USD", "john@doe.com", [itemUpdated]);
            updatedBillRequest.token = billToken;

            const result: BillInterface = await client.updateBill(updatedBillRequest, billId);
            expect(result.items[0].price).toBe(9.00);
        });
