...
describe("BitPaySDK.Client", () => {
  ...
  describe("Invoices", () => {
    ...
    it("should retrieve invoice list", async () => {
      ...
      let dateStart = new Date(date.setDate(date.getDate() - 30))
        .toISOString()
        .split("T")[0];
      ...
      retrievedInvoices = await client.GetInvoices(
        dateStart,
        dateEnd,
        status,
        null,
  ...
  describe("Refunds", () => {
    ...
    it("should get first paid invoice", async () => {
      firstPaidInvoice = await client.GetInvoices(
        dateStart,
        dateEnd,
    ...
    it("should create refund request", async () => {
      ...
      firstPaidInvoice = firstPaidInvoice.shift();
      createdRefund = await client.CreateRefund(
        firstPaidInvoice,
        refundEmail,
        firstPaidInvoice.price,
        firstPaidInvoice.currency
      );
  ...
  describe("Bills", () => {
    ...
    it("should create bill USD", async () => {
      let bill = new BitPaySDK.Models.Bill(
        "0001",
        Currencies.USD,
        "sandbox@bitpay.com",
        items
    ...
    it("should deliver bill", async () => {
      ...
      basicBillUsd = await client.CreateBill(bill);
      deliveredBill = await client.DeliverBill(
        basicBillUsd.id,
        basicBillUsd.token
      );
  ...
  describe("Ledger", () => {
    ...
    it("should retrieve ledger USD", async () => {
      ...
      retrievedLedger = await client.GetLedger(
        Currencies.USD,
        dateStart,
        dateEnd
      );
  ...
  describe("PayoutRecipients", () => {
    ...
    it("should get and update payout recipient", async () => {
      ...
      updatedRecipient = await client.UpdatePayoutRecipient(
        lastRecipient.id,
        "label.UPDATED",
        "https://hookb.in/1gw8aQxYQDHj002yk79K"
      );

      expect(lastRecipient).toBeDefined();
      expect(retrieved.id).toBeDefined();
      expect(updatedRecipient.label).toEqual("label.UPDATED");
      expect(updatedRecipient.notificationURL).toEqual(
        "https://hookb.in/1gw8aQxYQDHj002yk79K"
      );
    });
  ...
  describe("Payouts", () => {
    ...
    it("should request payout notification", async () => {
      ...
      createdPayout = await client.SubmitPayout(payout0);
      requestedNotification = await client.RequestPayoutNotification(
        createdPayout.id
      );
      cancelledPayout = await client.CancelPayout(createdPayout.id);
  ...
  describe("PayoutBatches", () => {
    ...
    it("should get payout batches by status", async () => {
      retrievedBatches = await client.GetPayoutBatches(
        null,
        null,
        PayoutStatus.New
      );
    ...
    it("should request payout batch notification", async () => {
      ...
      createdBatch = await client.SubmitPayoutBatch(batch0);
      requestedNotification = await client.RequestPayoutBatchNotification(
        createdBatch.id
      );
      canceledBatch = await client.CancelPayoutBatch(createdBatch.id);
  ...
  describe("Settlements", () => {
    ...
    it("should get settlements", async () => {
      retrievedSettlements = await client.GetSettlements(
        Currencies.USD,
        dateStart,
        dateEnd,
        null,
    ...
    it("should retrieve reconciliation report USD", async () => {
      ...
      firstSettlement = retrievedSettlements.shift();
      retrievedSettlement = await client.GetSettlementReconciliationReport(
        firstSettlement
      );
  ...
  describe("Subscriptions", () => {
    ...
    it("should get and update subscription", async () => {
      ...
      updatedSubscription = await client.UpdateSubscription(
