...
export class RESTcli {
  ...
  public constructor(environment: string, ecKey: elliptic.ec.KeyPair) {
    this._ecKey = ecKey;
    this._baseUrl =
      environment.toUpperCase() == Env.Test ? Env.TestUrl : Env.ProdUrl;
    this.init();
  }
  ...
  public async post(
    ...
    try {
      ...
      if (signatureRequired) {
        Object.assign(
          _options.headers,
          this.getSignedHeaders(_fullURL, _formData)
        );
      }
      ...
      throw new BitPayException(
        null,
        "RESTcli POST failed: " + JSON.stringify(e.response.data)
      );
    }
  }
  ...
  public async get(
    ...
    try {
      ...
      if (signatureRequired) {
        Object.assign(
          _options.headers,
          this.getSignedHeaders(_fullURL, _query)
        );
      }
      ...
      throw new BitPayException(
        null,
        "RESTcli GET failed : " + JSON.stringify(e.response.data)
      );
    }
  }

  public async delete(uri: string, parameters: any = {}): Promise<string> {
    try {
      const _fullURL = this._baseUrl + uri;
      const _options = JSON.parse(JSON.stringify(this._commonOptions));
      const _query = "?" + qs.stringify(parameters);
      ...
      throw new BitPayException(
        null,
        "RESTcli DELETE failed : " + JSON.stringify(e.response.data)
      );
    }
  }

  public async update(uri: string, formData: any = {}): Promise<string> {
    try {
      const _fullURL = this._baseUrl + uri;
      const _formData = JSON.stringify(formData);
      const _options = JSON.parse(JSON.stringify(this._commonOptions));
      ...
      throw new BitPayException(
        null,
        "RESTcli UPDATE failed : " + JSON.stringify(e.response.data)
      );
    }
  }
