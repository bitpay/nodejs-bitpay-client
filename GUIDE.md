...
If you use BitPay as a NPM package navigate to `node_modules/bitpay-sdk/dist/setup` and run the script using `node BitPaySetup.js` or you can enter the following command ```   node node_modules/bitpay-sdk/dist/setup/BitPaySetup.js ```. If you are using BitPay as a SDK then you will need to run `npm run build` and then there will be a dist folder generated. You need to navigate to `dist/setup` and run the setup script using ```node BitPaySetup.js``` or you can directly run by ```node dist/setup/BitPaySetup.js```
